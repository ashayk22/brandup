...
/**
 * LetterCascade from Componentry (https://componentry.dev, MIT).
 * Local changes: imports from "motion/react" and clsx; the hover effect fires
 * when the whole surrounding link or button is hovered (not just the letters);
 * reduced-motion visitors get plain static text.
 */
...
export function LetterCascade({
    ...
    // Listen on the closest link or button so the padding around the text counts too.
    useEffect(() => {
        if (triggerOnClick || reduceMotion) return;
        const host = (scope.current as HTMLElement | null)?.closest("a, button");
        if (!host) return;
        host.addEventListener("mouseenter", trigger);
        return () => host.removeEventListener("mouseenter", trigger);
    }, [scope, trigger, triggerOnClick, reduceMotion]);
