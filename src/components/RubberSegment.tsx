...
/**
 * RubberSegment from React Bits (https://reactbits.dev), TS + CSS variant.
 * Local changes, so it can drive the site navigation:
 *  - items may carry an `href` and then render as real <a> links (no radio
 *    semantics, no arrow-key selection; Tab and Enter work as for any link);
 *  - a `nav` size that matches the existing nav links (40px tall, 16px text);
 *  - when `value` matches no item nothing is highlighted (thumb fades out);
 *  - keyboard activation of a link animates the thumb like a tap does.
 */
...
export type RubberSegmentSize = 'sm' | 'md' | 'lg' | 'nav';
type NormalizedItem = { value: string; label: ReactNode; icon?: ReactNode; href?: string };
export type RubberSegmentItem = string | NormalizedItem;
...
const SIZES: Record<RubberSegmentSize, { height: number; font: number; pad: number; min: number }> = {
  ...
  nav: { height: 40, font: 16, pad: 20, min: 0 }
};
...
const RubberSegment: React.FC<RubberSegmentProps> = ({
  ...
  const list: NormalizedItem[] = items.map(item => (typeof item === 'string' ? { value: item, label: item } : item));
  const isLinks = list.some(item => !!item.href);
  const [inner, setInner] = useState<string | undefined>(defaultValue ?? list[0]?.value);
  const current = value !== undefined ? value : inner;
  const hasSelection = list.some(item => item.value === current);
  ...
  const commit = (i: number) => {
    committed.current = i;
    if (i === index && hasSelection) return;
  ...
  const handlePointerDown = (e: React.PointerEvent<HTMLElement>, i: number) => {
  ...
  const handlePointerUp = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    const x = localX(e);
    if (!d.live) {
      if (Math.abs(x - d.x0) <= SLOP && (d.slot !== committed.current || !hasSelection)) {
        const from = committed.current;
        commit(d.slot);
        if (hasSelection) travel(from, d.slot);
        else jumpTo(d.slot);
      }
      return;
    }
  ...
  const activateByKeyboard = (i: number) => {
    if (disabled || (i === committed.current && hasSelection)) return;
    const from = committed.current;
    commit(i);
    if (hasSelection) travel(from, i);
    else jumpTo(i);
  };

  const handleKeyDown = (e: React.KeyboardEvent<HTMLElement>) => {
    if (disabled || isLinks) return;
  ...
  return (
    ...
    >
      {list.map((item, i) =>
        item.href ? (
          <a
            ...
            href={item.href}
            aria-current={hasSelection && i === index ? 'location' : undefined}
            className="rubber-segment__item"
            draggable={false}
            onPointerDown={e => handlePointerDown(e, i)}
            onClick={e => {
              // detail 0 = keyboard activation (Enter); pointer taps are handled on pointer up
              if (e.detail === 0) activateByKeyboard(i);
            }}
          >
            {item.icon}
            {item.label}
      ...
      <motion.div className="rubber-segment__thumb" aria-hidden="true" style={{ clipPath, opacity: hasSelection ? 1 : 0 }}>
        {list.map(item => (
